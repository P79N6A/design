...
function replaceCont(filePath, fileCont) {
  fileCont = fileCont.replace(/<link(.*)href="([^"]*)"/g, function(
    match,
    p1,
    p2,
    offset,
    ...
    const relative = path.relative(
      srcDir,
      path.resolve(path.dirname(filePath), p2)
    )
    if (p2.indexOf('.') === 0) {
  ...
  fileCont = fileCont.replace(/src="([^"]*)"/g, function(
    match,
    p1,
    offset,
    string
    ...
    const relative = path.relative(
      srcDir,
      path.resolve(path.dirname(filePath), p1)
    )
